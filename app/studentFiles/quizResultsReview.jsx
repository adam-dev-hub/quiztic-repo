// studentFiles/quizResultsReview.jsx
...
// --- Main QuizResultsReviewScreen Component ---
function QuizResultsReviewScreen() {
  ...
  useEffect(() => {
    const fetchQuizResults = async () => {
      ...
      try {
        // 1. Fetch the specific quiz result for this student and session
        const resultRes = await databases.listDocuments(DATABASE_ID, QUIZ_RESULTS_COLLECTION, [
        ...
        if (resultRes.documents.length === 0) {
          setError("No quiz results found for this session.");
          setIsLoading(false);
          return;
        }

        const fetchedResult = resultRes.documents[0];
        setStudentQuizResult(fetchedResult);
        ...
        setAllQuestions(questionsRes.documents);

        // 3. Fetch all quiz results for comparison for this quiz
        ...
        console.error("Quiz Results Review Error:", err);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // Calculate student's metrics
  const studentOverallScore = studentQuizResult ? parseInt(studentQuizResult.final_score) : 0;
  const totalQuestions = studentQuizResult ? parseInt(studentQuizResult.total_questions) : 0;
  const totalCoins = studentQuizResult ? parseInt(studentQuizResult.total_coins) : 0;
        ...
        <View style={summaryStyles.headerContent}>
          <View style={summaryStyles.leftSection}>
            ...
            <View style={summaryStyles.textSection}>
              <SmartText style={[summaryStyles.headerTitle, { fontSize: 24 * textSizeMultiplier }]}>Quiz Results</SmartText>
              <SmartText style={[summaryStyles.headerSubtitle, { fontSize: 16 * textSizeMultiplier }]}>
                {studentQuizResult?.quiz_title || 'Review Your Performance'}
              </SmartText>
            </View>
          </View>
          <View style={summaryStyles.rightSection}>
            <View style={summaryStyles.lottieContainer}>
              <MaterialCommunityIcons name="clipboard-text-outline" size={50} color="rgba(255,255,255,0.9)" />
            </View>
          </View>
        </View>
      </LinearGradient>
            ...
            {allQuestions.map((question, index) => {
              ...
              return (
                <View key={question.$id} style={[summaryStyles.questionCard, { borderColor: colors.border }]}>
                  ...
                  {studentAnswerDetail.given_answer && (
                    <View style={summaryStyles.answerRow}>
                      ...
                      <SmartText style={[summaryStyles.answerText, { color: colors.text }]}>
                        {question.question_type === 'Fill-in-the-blank'
                          ? JSON.parse(studentAnswerDetail.given_answer).join(', ')
                          : studentAnswerDetail.given_answer}
                      </SmartText>
                    </View>
                  )}
...
export default function QuizResultsReviewScreenWrapper() {
  return (
    <AppProvider>
      <QuizResultsReviewScreen />
    </AppProvider>
  );
}